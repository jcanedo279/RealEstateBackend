...
def preprocess_dataframe(logger, df, filter_method=FilterMethod.FILTER_NONE, cols_to_keep=set()):
    ...
    logger.info("num cols: ", num_cols)
    logger.info("cat cols: ", cat_cols)
    ...
    logger.info("fitted cat_to_ohe_cols: ", preprocessor.cat_to_ohe_cols)
    logger.info("fitted ohe_to_cat_value: ", preprocessor.ohe_to_cat_value)
    df_preprocess = preprocessor.df_transform(df_preprocess)
    logger.finO("transformed")
